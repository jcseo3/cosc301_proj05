...
/*
//checks array to see if the value in an element is already 1 and deletes the current dir entry if so; else, it will increment the element 0 in the array to 1
void check_update_arr(int *refarr, uint16_t fatent){
	if (refarr[fatent] == 0){
		refarr[fatent]++;		
	}else{
...
int getclusterlen(uint16_t startCluster, uint8_t *image_buf, struct bpb33 *bpb, int *refarr){
	...
	//is the while loop not going to go through with a single bad cluster chain? 
	while (!is_end_of_file(fatent)){
	//if cluster is a bad cluster in the middle of a cluster chain, then set the previous cluster to EOF and make the rest of the clusters in the chain orphans
