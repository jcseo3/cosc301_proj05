...
/*
//checks array to see if the value in an element is already 1 and returns the current length of the cluster chain; else, it will increment the element 0 in the array to 1
void check_update_arr(int *refarr, uint16_t fatent, int clusterlen){
	if (refarr[fatent] == 0){
		refarr[fatent]++;		
	}else{
		return clusterlen; 
...
int getclusterlen(uint16_t startCluster, uint8_t *image_buf, struct bpb33 *bpb, int *refarr){
	...
	//if cluster is a bad cluster in the middle of a cluster chain, then set the previous cluster to EOF and make the rest of the clusters in the chain orphans
		if (fatent == (FAT12_MASK & CLUST_BAD)){
			...
			prevfat = fatent;
			//refarr[fatent]++;
			//checks to see if the array is already 1
			if (refarr[fatent] == 0){
				refarr[fatent]++;		
			}else{
				return clusterlen; 
